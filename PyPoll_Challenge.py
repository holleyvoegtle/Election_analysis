...
with open(file_to_load) as election_data:
    ...
    for row in reader:
        ...
        if fish not in county_names:

            # 4b: Add the existing county to the list of counties.
            county_names.append(fish) 
...
with open(file_to_save, "w") as txt_file:
    ...
    for fish in county_names:

        # 6b: Retrieve the county vote count.
        votes = county_votes.get(fish)
        # 6c: Calculate the percent of total votes for the county.
        vote_percentage = float(votes) / float(total_votes) * 100
        county_results = (
            f"{fish}: {vote_percentage:.1f}% ({votes:,})\n")
        ...
        if (votes > largest_county_turnout) and (vote_percentage > winning_county_percentage):
    ...
    winning_county_summary = (
        ...
        f"Winning Vote Count: {largest_county_turnout:,}\n"
